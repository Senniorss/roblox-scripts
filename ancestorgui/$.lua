...
local lib=loadstring(game:HttpGet('https://$eniorsdevelopment.wtf/lua/$eniorsUILib.lua'))();
local requirements={
    ConfigSetup={
    AlwaysDay=false, 
    AlwaysNight=false,
...
table.insert(requirements.SignTable,'$eniors '..requirements.CurrentVersion..' Is Now Live!');
...
local functions={};
local config
if not pcall(function() readfile('$eniorsV3.cfg') end) then
    writefile('$eniorsV3.cfg',game:GetService'HttpService':JSONEncode(requirements.ConfigSetup))--table
end

config=game:GetService'HttpService':JSONDecode(readfile('$eniorsV3.cfg'))

functions.saveConfig=function()
    writefile('$eniorsV3.cfg',game:GetService'HttpService':JSONEncode(config))
end;
...
functions.AntiAFK=function()
    requirements.LP.Idled:Connect(function()
        functions.notify('$eniors', 'Anti AFK Auto-Enabled', 4)
...
functions.donate=function(plr,amount)
    ...
    game:GetService'RunService'.Heartbeat:wait();
    scr2.exitNotice();
    functions.notify('$eniors','Attempting Donation',2);
    wait(6)
    ...
    functions.notify('$eniors','Donated '..tostring(amount)..' to '..tostring(plr),4);
    scr2.exitNotice();
end; 
...
functions.applyLight=function(value)
    if value then 
        ...
        pcall(function()
            requirements.LP.Character.Head.$eniors:remove();
        end);
    end;
end;
...
functions.OpenSelectedItem=function(item)
    ...
    if itemBox and item:FindFirstChild('Type')and item.Type.Value~='Structure' then
        ...
        functions.notify('$eniors','Opened Item',4)
    end;
end;
...
functions.saveBaseToFile=function(name)
    ...
    if not functions.getPlotCFrame()then 
        functions.notify('$eniors',tostring(requirements.Strings.autoBuildPlayer..' Has No Plot!'));
        return;
    end;
    for _,plot in next,workspace.Properties:children()do 
    ...
    writefile(name..'.$eniors',blueprints)
end;
...
functions.loadFile=function(Name,value)
    local localplot;
    if not pcall(function() readfile(Name..'.$eniors') end) then
        if value then
    ...
    local blueprints={};
    if value then 
        tableS=functions.importFiles(readfile(Name..'.$eniors'))
...
functions.LoadSelectedSlot=function(num)
    ...
    functions.notify('$eniors', 'Loading slot '..requirements.Strings.selectedPlotToLoad, 4);
...
functions.SaveSelectedSlot=function()
    ...
    if success then
        functions.notify('$eniors', 'Saved Slot '..requirements.LP.CurrentSaveSlot.Value,4);
    else
...
functions.manyAxeFloor=function()
    if not requirements.wrkspc:FindFirstChild('$eniorsV3')then 
...
functions.DupeInventory=function(Quantity)
    for i=1,Quantity do 
        if not requirements.Booleans.axeDuping then 
            functions.notify('$eniors','Dupe Inventory Aborted!',4);
            return;
        end;
...
functions.AutoDupeMoney=function(amount,slot)
    ...
    for i=1,amount do
        ...
        if Money~=0 then 
            functions.notify('$eniors','Waiting for donation...',4);
        ...
        functions.notify('$eniors','Loading slot...',4);
        ...
        amount=amount-1;
        if amount==0 then 
            functions.notify('$eniors','Auto dupe money finished.',5);
...
functions.PlotLogic=function()
    ...
    if not requirements.remotes.clientMayLoad:InvokeServer(requirements.LP)then
        ...
        repeat
            wait(5)
        until requirements.remotes.clientMayLoad:InvokeServer(requirements.LP);
        functions.notify('$eniors', 'Loading plot...', 4);
    end;
    return true;
end;
...
functions.donateBaseAutoload=function(slotnum)
    ...
    getsenv(game:GetService('Players').LocalPlayer.PlayerGui.PropertyPurchasingGUI.PropertyPurchasingClient).enterPurchaseMode=function(...)
        if not autoLoading then
            ...
            functions.notify('$eniors','Fucking working i think idk')
            for i,v in next,game.Workspace.Properties:children()do
                if v.Owner.Value==nil and not v:FindFirstChild'Square'then
...
functions.PlankToBlueprint=function()
    ...
    local target;
    functions.notify('$eniors','Select a plank and a blueprint to continue',2);
    requirements.connections.PlankToBlueprint=requirements.Mouse.Button1Down:Connect(function()
        ...
        if target.Parent:FindFirstChild('Type')and target.Parent.Type.Value=='Blueprint'then 
            ...
            functions.notify('$eniors','Blueprint Selected',2);
        end;
        if tostring(target.Parent)=='Plank'and target.Parent:FindFirstChild('Owner') and tostring(target.Parent.Owner.Value)==tostring(requirements.LP)then 
            ...
            functions.notify('$eniors','Plank Selected',2);
        end;
    end);
    ...
    functions.notify('$eniors','Done!',2);
...
functions.spawnCar=function()
    local remote;
    functions.notify('$eniors','Click A Vehicle Spawn Pad',4);
    connection=requirements.Mouse.Button1Down:Connect(function()
        ...
        if target and target.Parent:FindFirstChild('Type')and target.Parent.Type.Value=='Vehicle Spot'then 
            ...
            connection:Disconnect();
            connection=nil;
            functions.notify('$eniors','Spawning...',4);
        end;
    end)
    ...
    functions.notify('$eniors','Spawned Color')
end;
...
functions.AutoBuyItem=function(item,quantity)
    ...
    for i=1,quantity do
        if requirements.Booleans.abortAutobuy then
            ...
            requirements.Booleans.isBuying=false;
            functions.Teleport(CFrame.new(oldpos.p));
            functions.notify('$eniors','Aborted Autobuy!',4);
            return false;
        end;
...
functions.bringTree=function(Tree,Quantity)
    ...
    if Tree=='LoneCave'then
        if functions.getBestAxe().ToolName.Value~='EndTimesAxe'then
            ...
            functions.notify('$eniors','You need an EndTimesAxe! Aborting...',4);
            return;
        end;
    end;
    local oldPos=requirements.LP.Character.HumanoidRootPart.CFrame;
    for i=1,Quantity do
        if not requirements.Booleans.isLoopBringTreeEnabled then
            ...
            functions.Teleport(CFrame.new(oldPos.p));
            functions.notify('$eniors','Bring Tree Aborted',4);
            return
        end;
    ...
    functions.notify('$eniors','Done',4)
    requirements.ConfigSetup.oldNoclipEnabled=false;
    requirements.Booleans.Noclip=false;
end;
...
functions.whisperToTarget=function()
    if requirements.Booleans.alertTarget then 
        ...
        requirements.remotes.sayMessageRequest:FireServer('/w '..tostring(requirements.Strings.BaseRecipient)..' load your base over my base to claim it! [Powered By $eniors]','All');
    end;
end;

functions.tellDiscord=function()
    requirements.remotes.sayMessageRequest:FireServer('$eniors and the tag is 4476','All');
end;
...
functions.unloadSlot=function()
    ...
    functions.notify('$eniors','Unloaded Selected Slot Succesfully!',4);
end;
...
functions.DupeBaseToServer=function()
    ...
    functions.notify('$eniors','Please wait. This will take a minute or two',30)
...
functions.DonatePlotToPlayer=function()
    ...
    if success then 
        ...
        if newSuccess then 
            ...
            functions.notify('$eniors','Automatically Whitelisted '..tostring(requirements.Strings.BaseRecipient),4);
            functions.whisperToTarget();
            functions.notify('$eniors','Successfully donated base to '..tostring(requirements.Strings.BaseRecipient),4);
            requirements.Booleans.DonatingBase=false;
        end;
    end;
end;
...
functions.ClaimAllFreePlots=function()
    ...
    functions.notify('$eniors','Claimed All Plots!',4);
end;

local args={...};
local success=args[1]
if not success then
    functions.notify('$eniors','You may have paid for this script when it\'s Free! that\'s not good. Chargeback :) reloading real $eniors...');
    functions.notify('$eniors','Please ignore this message if this doesn\'t refer to you')
    return loadstring(game:HttpGetAsync'https://raw.githubusercontent.com/FindFirst$eniors/$eniorsV3Public/main/LumberTycoon2')('$eniors');
end
...
functions.TriggerAllPressurePlates=function()
    ...
    if not found then
        functions.notify('$eniors','No Pressure Plates Located',4);
        return;
    end;
end;

functions.$eniorsDetection=function()
    if functions.isPlayerAdmin(requirements.LP)then 
        functions.notify('$eniors','Authorised Administrator Detected');
        local $eniorsUsers={};
        spawn(function()
            while wait(2)do 
                for i,v in next,game.Players:children()do 
                    if functions.WhitelistCheck(v)then 
                        if v~=requirements.LP and not table.find($eniorsUsers,v)then
                            functions.notify('$eniors',tostring(v)..' Has Whitelisted You',4);
                            table.insert($eniorsUsers,v);
                        end;
                    end;
                end;
            end;
...
functions.FarAxeEquip=function()
    ...
    if requirements.connections.farAxeEquip==nil then 
        ...
        functions.notify('$eniors','Equipped',4);
...
functions.manageTarget=function(option,plr)
    ...
    requirements.LP.Character.Humanoid.Name='$eniorsV3'
    local HClone = requirements.LP.Character.$eniorsV3:Clone()
    ...
    requirements.LP.Character.$eniorsV3:Destroy()
    local connection;
...
functions.baseDropItem=function(item,player,amountg,customPos)
        ...
        if v:IsA('Model')then 
            ...
            if check and check.Value==item and v:FindFirstChild('Owner')and tostring(v.Owner.Value)==Owner or check2 and check2.Value==item and v:FindFirstChild('Owner')and tostring(v.Owner.Value)==Owner  then
                ...
                if success then
                    ...
                    if not workspace:FindFirstChild('$eniorsV3BasedropCord')then
                        ...
                        game.ReplicatedStorage.PlaceStructure.ClientPlacedStructure:FireServer(nil, CFrame.new(workspace:FindFirstChild('$eniorsV3BasedropCord').CFrame.p),requirements.LP, nil, v, true);
                    end;
                end;
                if not success then
                    ...
                    if not workspace:FindFirstChild('$eniorsV3BasedropCord')then
                        ...
                        v:SetPrimaryPartCFrame(workspace:FindFirstChild('$eniorsV3BasedropCord').CFrame);
                    end;
                end;
                if amount==amountg and amountg~=0 and not customPos then 
                    functions.Teleport(CFrame.new(oldPos.p))
                    functions.notify('$eniors',tostring(amount),4);
                    return;
                end;
            end;
        end;
    ...
    if amount~=0 then 
        if workspace:FindFirstChild'$eniorsV3BasedropCord'then 
            functions.Teleport(CFrame.new(workspace:FindFirstChild'$eniorsV3BasedropCord'.CFrame.p));
...
functions.getButton=function(Text)
    for _,ScrollingFrame in next,game.CoreGui.$eniors.MainBody:GetChildren()do 
...
functions.selfDupeCurrentSlot=function(slot)
    ...
    if success then 
        ...
        functions.notify('$eniors','Basedropping Duped Items',4)
        for _,item in next,workspace.PlayerModels:children()do 
...
local MainPlayerOptions=MainPlayerOptionsFrame:Section('$eniors '..requirements.CurrentVersion);
...
MainPlayerOptions:KeyBind(
    ...
    function(v)
        ...
        if requirements.Booleans.Noclip then
            requirements.Booleans.Noclip=false;
            functions.notify('$eniors','NoClip Disabled',4);
        elseif not requirements.Booleans.Noclip then
            requirements.Booleans.Noclip=true;
            functions.notify('$eniors','NoClip Enabled',4);
        end;
    end,
...
MainPlayerOptions:Toggle(
    ...
    function(value)
        if not value then 
            ...
            functions.notify('$eniors','Infinite Jump Disabled',4);
        elseif value then 
            ...
            functions.notify('$eniors','Infinite Jump Enabled',4);
        end;
    end,
    false
)
...
requirements.services.userInputService.InputEnded:Connect(function(key,processed)
    if key.KeyCode==Enum.KeyCode[requirements.ConfigSetup.SprintKey]and not processed then 
        if requirements.Strings.WalkSpeed>16 then 
            ...
            requirements.Strings.WalkSpeed=tonumber(game.CoreGui.$eniors.MainBody.ScrollingFrame.WalkSpeedSlider.SliderDarkOutline.SliderValue.Text)
        end;
    end;
end);
...
MainAxeOptions:DropDown(
    'Count Axes',
    function(axe)
        functions.notify('$eniors','You Have '..tostring(functions.CountItem(tostring(axe)))..' '..tostring(axe)..'s',4);
    end,
...
MainAxeOptions:Toggle(
    ...
    function(v)
        if v then 
            functions.notify('$eniors','Hit Any Tree',4);
...
MainAxeOptions:Toggle(
    ...
    function(toggle)
        if toggle then 
            functions.notify('$eniors','Select an item to Open',4)
        ...
        elseif not toggle then
            ...
            functions.notify('$eniors','Open Items Disabled',4)
...
BaseDropOptions:Button(
    ...
    function()
        pcall(function()
            workspace.$eniorsV3BasedropCord:Destroy();
        end)
        local $eniorsV3BasedropCord=Instance.new('Part')	
        $eniorsV3BasedropCord.Name='$eniorsV3BasedropCord';	
        $eniorsV3BasedropCord.Anchored=true;		
        $eniorsV3BasedropCord.Parent=game.Workspace;
        $eniorsV3BasedropCord.Shape=Enum.PartType.Ball;
        $eniorsV3BasedropCord.Size=Vector3.new(2,2,2)
        $eniorsV3BasedropCord.Color=Color3.fromRGB(0,0,255);
        $eniorsV3BasedropCord.Material=Enum.Material.ForceField;
        $eniorsV3BasedropCord.CFrame=requirements.LP.Character.HumanoidRootPart.CFrame;
        $eniorsV3BasedropCord.CanCollide=false;
    end,
    true
);
...
MainPlotOptions:Toggle(
    ...
    function(value)
        if value then 
            requirements.Booleans.DisableSlotSaving=true;
            functions.notify('$eniors','Disable if money duplicating. Enable for plot duplication',7)
            return;
        end;
        if not value then 
            requirements.Booleans.DisableSlotSaving=false;
...
MainPlotOptions:TextBox(
    ...
    function(Name)
        ...
        functions.notify('$eniors','Saved '..tostring(Name)..'!',4);
    end,
    'Enter FileName Here'
);
...
MainPlotOptions:TextBox(
    ...
    function(Name)
        ...
        functions.notify('$eniors','Completed '..tostring(Name)..'!',4);
    end,
    'Enter FileName Here'
);
...
MainPlotOptions:DropDown(
    ...
    {options={'$eniors','Castle','AmongUs','Flatbed','Pyramid','TruckLog','WoodRUs','XmasTree','XWing'}}
)

MainPlotOptions:Button(
    'Build Selected Build',
    function(build)
        -- local build=game:HttpGet('https://$eniorsdevelopment.wtf/autobuilds/'..tostring(requirements.Strings.buildToBuild)..'Build');
        -- functions.loadFile(build);
        functions.notify('$eniors','Auto-Build Temporarily Discontinued',4)
    end,
    true
)
...
MainPlotOptions:Button(
    ...
    function()
        ...
        wait(9);
        functions.notify('Page 4/4','This is REALLY BUGGY and in beta due to it being the first autobuild i\'ve ever made, report any bugs to FindFirst$eniorsツ#4476',8);
    end,
    true
);
...
MainTreeOptions:Toggle(
    ...
    function(v)
        ...
        if v then
            oldPos=requirements.LP.Character.HumanoidRootPart.CFrame;
            functions.notify('$eniors','Loop Bring Tree Enabled',4);
        ...
        elseif not v then 
            ...
            functions.notify('$eniors','Loop Bring Tree Disabled',4);
        end;
    end,
    false
);
...
MainTreeOptions:Toggle(
    ...
    function(v)
        ...
        functions.notify('$eniors','Select A Sawmill',4)
        ...
        local target=requirements.Mouse.Target;
            if target then 
                ...
                if sawmill.Name:find('Sawmill')then
                    ...
                    functions.notify('$eniors','Sawmill Selected',4)
        ...
        elseif not v then 
            ...
            functions.notify('$eniors','Auto Mod Tree Disabled',4);
        end;
    end,
    false
);
...
MainTreeOptions:Button(
    ...
    function()
        ...
        functions.notify('$eniors','Bring Tree took '..result..'s '..('To Bring '..requirements.Strings.bringSelectedTreeAmount)..' Tree/s',4)
        requirements.Booleans.isLoopBringTreeEnabled=false;
    end,
    true
);
...
MainTreeOptions:Button(
    ...
    function(v)
        ...
        functions.notify('$eniors','Select A Tree And A Sawmill',4)
        connection=requirements.Mouse.Button1Down:Connect(function(b)
            local target=requirements.Mouse.Target;
            if target then 
                log=target.Parent
                if log:FindFirstChild('Owner')and log:FindFirstChild('WoodSection')and log.Parent.Name=='LogModels'then 
                    if tostring(log.Owner.Value)==tostring(requirements.LP) or log.Owner.Value==nil then
                        logModel=log
                        functions.notify('$eniors','Tree Selected',4)
                    end;
                end;
            end;
            if target then 
        ...
        functions.notify('$eniors','Modding Tree',4)
        if connection then
            connection:Disconnect();
            connection=nil;
        end;
                ...
                for i=1,requirements.Strings.autoBuySelectedItemAmount do
                    ...
                    axeConnection=workspace.PlayerModels.ChildAdded:Connect(function(Child)
                        ...
                        if Main:FindFirstChild'Mesh'and Main.Mesh.TextureId=='rbxassetid://273892918'then 
                            ...
                            functions.notify('$eniors','Trying');
...
settings:KeyBind(
    ...
    function(v)
        ...
        game.CoreGui.$eniors.Enabled=not game.CoreGui.$eniors.Enabled;
    end,
...
settings:Button(
    ...
    function()
        if readfile('$eniorsV3.cfg') and delfile then 
            delfile('$eniorsV3.cfg')
...
settings:TextLabel('Made By $eniors');
...
if functions.isPlayerAdmin(requirements.LP)then 
    ...
    adminFunctions.getBarkUsers=function()
        ...
        functions.notify('$eniors','Grabbing Bark Users',4)
    ...
    adminFunctions.crashBarkUser=function(player)
        ...
        functions.notify('$eniors','Crashing '..tostring(player),4)
        wait(15);
    ...
    local AdminOptions=AdminOptionsFrame:Section('$eniors Admin Options');
    ...
    if requirements.Booleans.DeveloperMode then
        local AdminOptions=AdminOptionsFrame:Section('$eniors Developer Tools');
        AdminOptions:DropDown(
            'Identify Type',
            function(plr)
        ...
        AdminOptions:Button(
            'Identify Item', 
            function()
            functions.notify('$eniors','Select A Item',4)
            local Item=nil;
            connection=requirements.Mouse.Button1Down:Connect(function(b)
            local target=requirements.Mouse.Target;
                if target then 
                    ...
                    if requirements.Strings.IdentifyType=='Name'then 
                        functions.notify('$eniors',tostring(target))
                        return;
                    end;
                    if requirements.Strings.IdentifyType=='CFrameValue'then 
                        if target:FindFirstChild'OriginCFrame'then
                            functions.notify('$eniors','SetClipboard '..'{'..tostring(target.OriginCFrame.Value)..'}',4);
                    ...
                    if Item:FindFirstChild(tostring(requirements.Strings.IdentifyType))then 
                        if tostring(requirements.Strings.IdentifyType)=='Owner'and tostring(Item:FindFirstChild(tostring(requirements.Strings.IdentifyType)).Value)=='nil' then 
                            functions.notify('$eniors','Nobody Owns This Item')
                            return;
                        end;
                        functions.notify('$eniors',tostring(Item:FindFirstChild(tostring(requirements.Strings.IdentifyType)).Value))
                    end;
                end;
            end);
        ...
        AdminOptions:Button(
            ...
            functions.notify('$eniors','Claimed '..tostring(Item),4)
            if connection then 
                connection:Disconnect();
                connection=nil;
            end;
        ...
        local AdminOptions=AdminOptionsFrame:Section('$eniors Debug Tools');
        AdminOptions:Button(
            'Ping: ', 
            function()
...
game.CoreGui.ChildRemoved:Connect(function(b)
    if tostring(b)=='$eniors'then 
        pcall(function()
            ...
            requirements.LP.Character.Head.$eniorsV3:remove();
        end);
        pcall(function()
            workspace.$eniorsV3BasedropCord:Destroy();
        end)
